...
import { prepareThirdPartyPlugins, exportThirdPartyPlugins, inspectThirdPartyPlugins, readInstalledThirdPartyPlugins, syncThirdPartyPlugins } from '../scripts/sync-third-party-plugins.mjs'
import { exportPrivateEnvironment, importPrivateEnvironment, preparePrivateEnvironment, privateEnvironmentPaths } from '../scripts/private-environment-sync.mjs'
...
export async function publishPrivateEnvironmentRepository(config, { dshHome, profile, spawnCommand = spawn } = {}) {
  ...
  prepareThirdPartyPlugins({ profileDir: resolvePrivatePluginProfileDir(dshHome, profile), profile })
...
export async function importPrivateEnvironmentRepository(config, { dshHome, profile, spawnCommand = spawn } = {}) {
  ...
  await preparePrivateEnvironment({ dshHomePath: dshHome, dataRootPath: dataConfig.localPath, profile })
  ...
  outcome.operation.message = `私有环境已应用；插件已对齐：${String(thirdParty.plugins.length)} 个。`
  return outcome
}
