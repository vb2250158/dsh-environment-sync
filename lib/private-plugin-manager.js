...
import { deferRestart as writeDeferredRestart, prepareThirdPartyPlugins, exportThirdPartyPlugins, inspectThirdPartyPlugins, readInstalledThirdPartyPlugins, recordPluginBaseline, syncThirdPartyPlugins } from '../scripts/sync-third-party-plugins.mjs'
import { exportPrivateEnvironment, preparePrivateEnvironment, privateEnvironmentPaths, PRIVATE_SYNC_KEY_FILENAME } from '../scripts/private-environment-sync.mjs'
import { chooseEnvironmentConflict, environmentConflicts, environmentConflictDetails, environmentState, markEnvironmentRestored, synchronizeEnvironment } from './environment-workflow.js'
import { applyEnvironment } from './environment-apply.js'
...
const RUNTIME_FILES = ['package.json', 'lib/private-plugin-manager.js', 'lib/environment-apply.js', 'lib/environment-workflow.js', 'scripts/private-environment-sync.mjs', 'scripts/sync-third-party-plugins.mjs']
function runtimeDigest(root) {
  if (RUNTIME_FILES.some(file => !existsSync(join(root, file)))) return null
  const hash = createHash('sha256')
  for (const file of RUNTIME_FILES) hash.update(file).update(readFileSync(join(root, file)))
  return hash.digest('hex')
}
const LOADED_DIGEST = runtimeDigest(PACKAGE_ROOT)
const LOADED_VERSION = JSON.parse(readFileSync(join(PACKAGE_ROOT, 'package.json'), 'utf8')).version
...
function patchEntry(node) {
  ...
  return {
    id,
    enabled: typeof disabled === 'boolean' ? !disabled : null,
...
function lastPatchState(contents, path, loaderId) {
  const states = patchStates(contents, path, loaderId).filter(state => state.enabled !== null)
...
/** Catalog scope values: a session-scoped plugin changes one conversation; a global one changes the whole product. */
export const CATALOG_SCOPES = ['session', 'global']
export const DEFAULT_CATALOG_SCOPE = 'global'

function readCatalogScope(value) {
  return CATALOG_SCOPES.includes(value) ? value : DEFAULT_CATALOG_SCOPE
}
...
function readCatalog(catalogPath = CATALOG_PATH) {
  ...
  return catalog.plugins
    ...
    .map(plugin => ({
      ...
      scope: readCatalogScope(plugin.scope),
...
function profileInstalledPlugins(state) {
  let installed
  try {
    installed = readInstalledThirdPartyPlugins(state.profileDir)
  } catch {
    installed = []
  }
  if (state.package.installed) {
    installed.push({
      name: PRIVATE_PLUGIN_PACKAGE_NAME,
      version: state.package.installedVersion,
      description: '',
      author: null,
    })
  }
...
function installedPluginStatus(plugin, state, profile, installed) {
  const localVersion = installed.version ?? installedPluginVersion(state.profileDir, plugin.packageName)
  ...
  const enablement = privatePluginEnablement(state.dshHome, profile, plugin.loaderId)
  return {
    ...plugin,
    localVersion,
    remoteVersion: null,
    ...
    controlAvailable: !enablement.homeHasOverride,
    ...enablement,
  }
}

function discoveredPluginStatus(installed) {
  return {
    id: installed.name,
    name: installed.name,
...
export function readPrivatePluginStatus({
  ...
  const state = { ...profilePackageState(dshHome, safeProfile), dshHome }
  const catalog = new Map(readCatalog(catalogPath).map(plugin => [plugin.packageName, plugin]))
  const plugins = profileInstalledPlugins(state).map(installed => {
    const plugin = catalog.get(installed.name)
    return plugin === undefined
      ? discoveredPluginStatus(installed)
      : installedPluginStatus(plugin, state, safeProfile, installed)
  })
...
/** Run one child process and return bounded diagnostic output. */
export function runProcess(command, args, { cwd, spawnCommand = spawn, rawOutput = false } = {}) {
...
export async function publishPrivateEnvironmentRepository(config, { dshHome, profile, spawnCommand = spawn } = {}) {
  return runEnvironmentWorkflow(config, { dshHome, profile, spawnCommand, upload: true })
}
...
export async function importPrivateEnvironmentRepository(config, { dshHome, profile, spawnCommand = spawn } = {}) {
  return runEnvironmentWorkflow(config, { dshHome, profile, spawnCommand, upload: false })
}

async function runEnvironmentWorkflow(config, { dshHome, profile, spawnCommand, upload }) {
  ...
  const pluginOptions = { profileDir: resolvePrivatePluginProfileDir(dshHome, profile), repositoryPath: dataConfig.localPath, profile }
  const environmentOptions = { dshHomePath: dshHome, dataRootPath: dataConfig.localPath, profile }
  const operation = await synchronizeEnvironment({
    home: dshHome, profile, repository: dataConfig.localPath, upload,
    exportSnapshot: async () => {
      prepareThirdPartyPlugins(pluginOptions)
      await exportPrivateEnvironment(environmentOptions)
      exportThirdPartyPlugins(pluginOptions)
    },
    applySnapshot: async () => {
      await applyEnvironment(environmentOptions)
      recordPluginBaseline(pluginOptions)
    },
  })
  return { repository: await inspectRepository(dataConfig, spawnCommand), operation: { ...operation, action: upload ? 'publish-data' : 'sync-data' } }
}
...
function createManagerClass(protocol) {
  ...
  class Manager extends protocol.TypertRemoteService {
    ...
    async fetchData() { return this.runDataRepository('fetch') }
    ...
    async resolveConflict(request) {
      ...
      const config = this.repositoryConfig()
      if (config === undefined) throw new Error('请先配置私有环境仓库')
      const remaining = await chooseEnvironmentConflict({ home: this.dshHome, profile: this.profile, repository: config.dataLocalPath, path: request.path, side: request.side })
      if (!remaining.length) return this.syncData()
      return this.snapshot(config)
    }

    async setSyncKey(request) {
      if (this.running) throw new Error('Private environment operation is already running')
      if (typeof request.key !== 'string' || request.key.trim().length < 16) throw new Error('同步密钥至少需要 16 个字符')
      const config = this.repositoryConfig()
      if (config && existsSync(join(config.dataLocalPath, 'environment.json'))) {
        await preparePrivateEnvironment({ dshHomePath: this.dshHome, dataRootPath: config.dataLocalPath, profile: this.profile, encryptionSecret: request.key.trim() })
      }
      writeFileSync(join(this.dshHome, PRIVATE_SYNC_KEY_FILENAME), request.key.trim() + '\n', { mode: 0o600 })
      this.lastOperation = { state: 'succeeded', action: 'set-key', message: '同步密钥已保存在本机。' }
      return this.snapshot()
    }

    async changePlugin(request) {
      if (this.running) throw new Error('Private environment operation is already running')
      if (!['add', 'remove'].includes(request.action) || typeof request.value !== 'string') throw new Error('Invalid plugin action')
      const profileDir = resolvePrivatePluginProfileDir(this.dshHome, this.profile)
      const snapshot = prepareThirdPartyPlugins({ profileDir, profile: this.profile })
      const repositoryPath = join(profileDir, '.dsh-plugin-requests', randomUUID())
      mkdirSync(join(repositoryPath, 'config'), { recursive: true })
      this.running = true
      try {
        await exportPrivateEnvironment({ dshHomePath: this.dshHome, dataRootPath: repositoryPath, profile: this.profile })
        if (request.action === 'remove') {
          if (request.value === PRIVATE_PLUGIN_PACKAGE_NAME || !snapshot.plugins.some(plugin => plugin.name === request.value)) throw new Error('该插件不能从此入口移除')
          writeFileSync(join(repositoryPath, 'config', 'plugins.json'), JSON.stringify(snapshot))
          recordPluginBaseline({ profileDir, repositoryPath, profile: this.profile })
          snapshot.plugins = snapshot.plugins.filter(plugin => plugin.name !== request.value)
        } else {
          const match = /^(?:https:\/\/github\.com\/|github:)?([A-Za-z0-9_.-]+)\/([A-Za-z0-9_.-]+?)(?:\.git)?(?:#([0-9a-f]{40}))?$/i.exec(request.value.trim())
          if (match === null) throw new Error('请输入 GitHub owner/repository 或仓库网址，可选 #完整提交号')
          const remote = `https://github.com/${match[1]}/${match[2]}.git`
          const initialized = await git(repositoryPath, ['init'])
          if (!initialized.ok) throw new Error(initialized.output)
          const fetched = await git(repositoryPath, ['fetch', '--depth=1', remote, match[3] ?? 'HEAD'])
          if (!fetched.ok) throw new Error(fetched.output || '无法读取插件仓库，请检查 GitHub 授权')
          const head = await gitValue(repositoryPath, ['rev-parse', 'FETCH_HEAD'])
          const packageResult = await runProcess('git', ['-C', repositoryPath, 'show', 'FETCH_HEAD:package.json'], { rawOutput: true })
          if (!packageResult.ok) throw new Error('插件仓库根目录缺少 package.json')
          const manifest = JSON.parse(packageResult.output)
          if (!manifest.dsh?.bundle && !manifest.dsh?.client) throw new Error('该仓库不是可安装的 DSH 插件')
          const author = typeof manifest.author === 'string' ? manifest.author : manifest.author?.name
          const plugin = { name: manifest.name, version: manifest.version, specifier: `github:${match[1]}/${match[2]}#${head}`, source: 'github', description: manifest.description ?? '', author: author ?? match[1], ...(manifest.dsh.upstreamRepository ? { upstreamRepository: manifest.dsh.upstreamRepository } : {}) }
          snapshot.plugins = [...snapshot.plugins.filter(item => item.name !== plugin.name), plugin]
        }
        writeFileSync(join(repositoryPath, 'config', 'plugins.json'), JSON.stringify(snapshot, null, 2) + '\n')
        await applyEnvironment({ dshHomePath: this.dshHome, dataRootPath: repositoryPath, profile: this.profile })
        this.lastOperation = { state: 'succeeded', action: 'change-plugin', message: '插件已更新，上传改动后可同步到其他电脑。' }
    ...
    async restoreEnvironment() {
      ...
      this.running = true
      try {
        await applyEnvironment({ dshHomePath: this.dshHome, dataRootPath: config?.dataLocalPath ?? resolvePrivatePluginProfileDir(this.dshHome, this.profile), profile: this.profile, restoreOnly: true })
        markEnvironmentRestored(this.dshHome, this.profile)
        writeFileSync(join(resolvePrivatePluginProfileDir(this.dshHome, this.profile), '.dsh-restart-required'), '{}\n')
        this.lastOperation = { state: 'succeeded', action: 'restore', message: '已恢复上次环境，请重启加载。' }
      } finally { this.running = false }
      return this.snapshot(config)
    }

    async deferRestart() {
      if (this.running) throw new Error('同步进行中，请稍后标记重启')
      writeDeferredRestart(resolvePrivatePluginProfileDir(this.dshHome, this.profile), this.profile)
      this.lastOperation = { state: 'succeeded', action: 'defer-restart', message: '已缓存待生效更新；重启 DSH 后加载全部变化。' }
      return this.snapshot()
    }

    async restartEnvironment() {
      if (this.running) throw new Error('同步进行中，请稍后重启')
      if (process.platform !== 'win32') throw new Error('当前重启入口仅支持 Windows，请使用本机 DSH 启动器重启')
      const sourceRoot = process.env.DSH_SOURCE_ROOT || process.cwd()
      if (!existsSync(join(sourceRoot, 'apps', 'cli', 'src', 'bin.ts'))) throw new Error('未配置 DSH_SOURCE_ROOT，无法自动重启')
      const nativeHome = realpathSync.native(this.dshHome)
      const helperRoot = join(nativeHome, 'recovery-runtime')
      mkdirSync(helperRoot, { recursive: true })
      const script = join(helperRoot, 'restart-dsh-web.ps1')
      const installedScript = join(resolvePrivatePluginProfileDir(this.dshHome, this.profile), 'node_modules', PRIVATE_PLUGIN_PACKAGE_NAME, 'scripts', 'ensure-dsh-web.ps1')
      writeFileSync(script, readFileSync(installedScript))
      const log = join(helperRoot, 'restart-request.log')
      const launcher = join(helperRoot, 'request-restart.ps1')
      writeFileSync(launcher, `Start-Sleep -Seconds 2\n& '${script.replaceAll("'", "''")}' -Restart\n`)
      const launchLog = join(helperRoot, 'restart-launch.log')
      const output = openSync(launchLog, 'a')
      let child
      try {
        const quote = value => "'" + value.replaceAll("'", "''") + "'"
        const launch = `$ErrorActionPreference = 'Stop'; Start-Process -WindowStyle Hidden -FilePath (Join-Path $PSHOME 'powershell.exe') -ArgumentList @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', ${quote('"' + launcher + '"')}) -RedirectStandardOutput ${quote(log)} -RedirectStandardError ${quote(log + '.error')}`
        child = spawn('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', launch], {
          windowsHide: true, stdio: ['ignore', output, output], env: { ...process.env, DSH_HOME: nativeHome, DSH_SOURCE_ROOT: realpathSync.native(sourceRoot), DSH_WEB_PROFILE: this.profile },
        })
      } finally { closeSync(output) }
      await new Promise((resolve, reject) => {
        child.once('error', reject)
        child.once('close', code => code === 0 ? resolve() : reject(new Error(`重启启动失败，请查看 ${launchLog}`)))
      })
      this.lastOperation = { state: 'succeeded', action: 'restart', message: '正在重启，请稍后刷新页面。' }
      return this.snapshot()
    }
    ...
    async recordThirdParty() {
      ...
      try {
        ...
        this.lastOperation = {
          ...
          message: `已记录 ${String(recorded.plugins.length)} 个插件；提交并推送私有仓库后，其他电脑可同步安装。`,
        }
      } catch (error) {
        this.lastOperation = {
          state: 'failed',
    ...
    async syncThirdParty() {
      ...
      try {
        ...
        this.lastOperation = {
          ...
          message: `插件已对齐：${String(result.plugins.length)} 个。`,
        }
      } catch (error) {
        this.lastOperation = {
          state: 'failed',
    ...
    async snapshot(config = this.repositoryConfig()) {
      ...
      const installedDigest = runtimeDigest(join(base.profileDir, 'node_modules', PRIVATE_PLUGIN_PACKAGE_NAME))
      const runtime = { version: LOADED_VERSION, processId: process.pid, digest: LOADED_DIGEST, installedDigest, matchesInstalled: installedDigest !== null && installedDigest === LOADED_DIGEST }
      let thirdParty
      if (config === undefined) {
      ...
      return {
        ...
        thirdParty,
        conflicts: config === undefined ? [] : await environmentConflicts(config.dataLocalPath),
        conflictDetails: config === undefined ? [] : await environmentConflictDetails(config.dataLocalPath),
        syncState: environmentState(this.dshHome, this.profile),
        operation,
        runtime,
        restartRequired: existsSync(join(base.profileDir, '.dsh-restart-required')) || (installedDigest !== null && !runtime.matchesInstalled),
        keyConfigured: existsSync(join(this.dshHome, PRIVATE_SYNC_KEY_FILENAME)),
        canRestore: existsSync(join(base.profileDir, '.dsh-environment-restore.json')),
  ...
  markRemoteMethod('fetchData')
  markRemoteMethod('publishData')
  markRemoteMethod('syncData')
  markRemoteMethod('resolveConflict')
  markRemoteMethod('setSyncKey')
  markRemoteMethod('changePlugin')
  markRemoteMethod('restoreEnvironment')
  markRemoteMethod('deferRestart')
  markRemoteMethod('restartEnvironment')
