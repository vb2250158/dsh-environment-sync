...
function createManagerClass(protocol) {
  ...
  class Manager extends protocol.TypertRemoteService {
    ...
    async restartEnvironment() {
      ...
      const script = fileURLToPath(new URL('../scripts/ensure-dsh-web.ps1', import.meta.url))
      const child = spawn('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', `Start-Sleep -Seconds 2; & '${script.replaceAll("'", "''")}'`], {
    ...
    async snapshot(config = this.repositoryConfig()) {
      ...
      return {
        ...
        operation,
        restartRequired: existsSync(join(base.profileDir, '.dsh-restart-required')),
