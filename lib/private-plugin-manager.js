...
function createManagerClass(protocol) {
  ...
  class Manager extends protocol.TypertRemoteService {
    ...
    async restartEnvironment() {
      ...
      const helperRoot = join(this.dshHome, 'recovery-runtime')
      ...
      const child = spawn('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', `Start-Sleep -Seconds 2; & '${script.replaceAll("'", "''")}' -Restart *> '${log.replaceAll("'", "''")}'`], {
        detached: true, windowsHide: true, stdio: 'ignore', env: { ...process.env, DSH_HOME: this.dshHome, DSH_SOURCE_ROOT: sourceRoot, DSH_WEB_PROFILE: this.profile },
      })
