window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    let cache
    function PluginManagerSection({ readStatus, configure, setEnabled, cloneData, fetchData, publishData, syncData, recordPlugins, syncPlugins }) {
      ...
      return React.createElement('div', { style: { display: 'grid', gap: '14px', padding: '8px 0' } },
        ...
        dataRepository === undefined
          ...
          : React.createElement('section', { style: card },
              ...
              React.createElement(Button, { variant: 'outline', type: 'button', disabled: busy !== '' || dataRepository.isGitRepository !== true || dataRepository.changes !== 0, onClick: () => { void run('publish-data', publishData) }, style: button }, busy === 'publish-data' ? '正在上传…' : '上传改动'),
              React.createElement(Button, { variant: 'outline', type: 'button', disabled: busy !== '' || dataRepository.isGitRepository !== true || dataRepository.changes !== 0, onClick: () => { void run('sync-data', syncData) }, style: button }, busy === 'sync-data' ? '正在应用…' : '拉取并应用'),
        ...
        React.createElement('section', { style: card },
          ...
          React.createElement('div', { style: { display: 'grid', gap: '8px', marginTop: '12px' } }, installed.map(plugin => React.createElement(PluginRow, { key: plugin.id, plugin, busy, setEnabled, run }))),
        ),
        React.createElement('section', { style: card },
