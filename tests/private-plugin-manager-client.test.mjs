...
function context() {
  ...
  const ctx = {
    ...
    reflect: { get: () => ({ status: response, configure: response, setEnabled: response, cloneData: response, fetchData: response, publishData: response, syncData: response, resolveConflict: response, setSyncKey: response, changePlugin: response, restoreEnvironment: response, deferRestart: response, restartEnvironment: response, recordThirdParty: response, syncThirdParty: response }) },
    slots: {
...
test('客户端只配置私有仓库，并提供按清单拉取插件的操作', async () => {
  ...
  assert.deepEqual(mounted[0].descriptors.map(item => item.method), ['status', 'configure', 'setEnabled', 'cloneData', 'fetchData', 'publishData', 'syncData', 'resolveConflict', 'setSyncKey', 'changePlugin', 'restoreEnvironment', 'deferRestart', 'restartEnvironment', 'recordThirdParty', 'syncThirdParty'])
